...
Future<List<Location>> searchLocations(String query, {int limit = 10}) async {
    try {
    final Uri uri = Uri.parse('$_baseUrl/search')
...
Future<Location> getLocationDetails({String? placeId, double? latitude, double? longitude}) async {
    try {
        ...
        final Uri uri = Uri.parse('$_baseUrl/details')
            .replace(queryParameters: {
            'place_id': placeId,
            'format': 'json',
        });
    ...
    if (response.statusCode == 200) {
        ...
        throw Exception('Failed to get location details: ${response.statusCode}');
    }
    } catch (e) {
...
try {
    final Uri uri = Uri.parse('$_baseUrl/reverse')
        .replace(queryParameters: {
...
Future<List<Location>> getNearbyPlaces(
    double latitude, double longitude, {String type = '', double radius = 1000}) async {
    try {
    final Uri uri = Uri.parse('$_baseUrl/search')
        .replace(queryParameters: {
        'format': 'json',
